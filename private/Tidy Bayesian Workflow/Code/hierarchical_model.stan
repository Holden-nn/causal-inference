...
generated quantities {
  ...
  // vector[N] mu;                          // Copy of mu for model comparison.
  // vector[N] log_lik;                     // Declare log_lik for model comparison.
  ...
  for (n in 1:N) {
    ...
    // // Save out the log likelihood for model comparison.
    // mu[n] = alpha[g[n], brand[n]] + beta[g[n]] * price[n];
    // log_lik[n] = normal_lpdf(intent[n] | mu[n], sigma);
  }
}
